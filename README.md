...
Garmin Edge records the ride. Strava handles the social layer. Segment Analyzer explains the ride: deeper split analysis, gradient-aware maps, and ride-vs-ride comparison than either of those tools surfaces on their own. Analysis runs entirely on-device — the only network calls are for import and map tiles.
...
- **Import rides** from Garmin Connect (SSO login), a local `.fit` file (official Garmin FIT SDK), or a local `.gpx` file
- **Sync starred Strava segments** (OAuth), including full route geometry
- **Automatic segment matching** — finds every point a ride's GPS track passed through a known segment, including multiple laps within one ride
- **Segment Detail**: personal best, progress-over-time chart, all past attempts
...
- **Offline-first**: everything except import and map tiles works without a network connection
